...
def read_reference_data_wmt(lt, ls):
	...
	for idx in range(len(liness)):
		...
		if "</seg>" in lines:
			...
			linet = linet.replace(" > ", " ")
			linet = linet.replace(" & ", " ")
			docs = xmltodict.parse(lines)
			doct = xmltodict.parse(linet)
			id = docs['seg']['@id']

			lines = re.sub('<seg.*?">', '', lines, flags=re.DOTALL)
			while "<term " in lines:
				lines = re.sub('<term.*?"> ', '', lines, flags=re.DOTALL)
				lines = lines.replace("</term> ", "")
			source = ' ' + ' '.join(lines.replace("</seg>", "").strip().split()) + ' '
			linet = re.sub('<seg.*?">', '', linet, flags=re.DOTALL)
			while "<term " in linet:
				linet = re.sub('<term.*?"> ', '', linet, flags=re.DOTALL)
				linet = linet.replace("</term> ", "")
			target = ' ' + ' '.join(linet.replace("</seg>", "").strip().split()) + ' '
			source_tokens = source.split()
			target_tokens = target.split()

			terms = []
			terms_l = []
			if 'term' in doct['seg'] and '@id' in doct['seg']['term']:
				src_start = source_tokens.index(docs['seg']['term']['#text'].split()[0])
				src_end = source_tokens.index(docs['seg']['term']['#text'].split()[-1])
				src_ids = ""
				for ind in range(src_start, src_end - 1):
					src_ids += (str)(ind) + ","
				src_ids += (str)(src_end)
				tgt_start = target_tokens.index(doct['seg']['term']['#text'].split()[0])
				tgt_end = target_tokens.index(doct['seg']['term']['#text'].split()[-1])
				tgt_ids = ""
				...
				for index in range(len(doct['seg']['term'])):
					...
					src_end = source_tokens.index(docs['seg']['term'][index]['#text'].split()[-1])
					src_ids = ""
					for ind in range(src_start, src_end - 1):
						src_ids += (str)(ind) + ","
					src_ids += (str)(src_end)
					tgt_start = target_tokens.index(doct['seg']['term'][index]['#text'].split()[0])
					tgt_end = target_tokens.index(doct['seg']['term'][index]['#text'].split()[-1])
					tgt_ids = ""
					for ind in range(tgt_start, tgt_end - 1):
						tgt_ids += (str)(ind) + ","
					tgt_ids += (str)(tgt_end)
	...
	for linet in linest:
		if "</seg>" in linet:
			...
			while "<term " in linet:
				linet = re.sub('<term.*?"> ', '', linet, flags=re.DOTALL)
				linet = linet.replace("</term> ", "")
			linet = linet.replace("</seg>", "")
			outputs.append(' ' + ' '.join(linet.strip().split()) + ' ')
	return outputs, refs

def read_reference_data_wmt_ENG(lt, ls):
	with open(lt, encoding="utf-8") as inp:
		linest = inp.readlines()
	with open(ls, encoding="utf-8") as inp:
		liness = inp.readlines()
...
def read_outputs_wmt(f):
	...
	for line in lines:
		if "</seg>" in line:
			...
			ids.append(doc['seg']['@id'])
			outputs.append(' ' + ' '.join(doc['seg']['#text'].strip().split()) + ' ')
	return ids, outputs
